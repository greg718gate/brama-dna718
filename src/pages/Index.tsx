...
const Index = () => {
  ...
  const isOperatorAdmin = operator?.email?.toLowerCase() === "grzegorzniepsuj47@gmail.com";
    ...
    <div className="relative min-h-screen overflow-y-auto">
      ...
      >
        ...
        <Button
          ...
          aria-disabled={!isOperatorAdmin}
          onClick={
            isOperatorAdmin
              ? () => {
                  setActiveTab("start");
                  window.dispatchEvent(new Event("sentinel-open-pro"));
                }
              : undefined
          }
          title={
          ...
          variant="outline"
          className={`col-span-2 h-10 w-full whitespace-normal border-premium/80 bg-premium/10 px-3 text-xs font-bold text-premium md:w-auto md:text-sm ${isOperatorAdmin ? "" : "opacity-80"}`}
        >
